...
def get_all_music_num():
    with connection.cursor() as cursor:
        ...
        cursor.execute(sql, ())
        return cursor.fetchone()
def get_all_music_num2():
    with connection.cursor() as cursor:
        sql = "SELECT count(1) as num FROM `musics2`  where music_id >= 0"
        cursor.execute(sql, ())
        return cursor.fetchone()
...
def insert_music2(music_id, music_name, nickname):
    with connection.cursor() as cursor:
        sql = "INSERT INTO `musics2` (`music_id`, `music_name`, `nickname`) VALUES (%s, %s, %s)"
...
def get_all_music2():
    with connection.cursor() as cursor:
        sql = "SELECT `music_id` FROM `musics2` ORDER BY music_id"
        cursor.execute(sql, ())
        return cursor.fetchall()
